...
@dataclasses.dataclass
class ToolbarActionMessage(Message, include_in_scene_serialization=False):
    """Message from client->server when a toolbar action is triggered."""

    action: Literal[
        "reframe_view", "zoom_in", "zoom_out", "perspective_view", "snapshot"
    ]
...
@dataclasses.dataclass
class ToolbarConfigMessage(Message, include_in_scene_serialization=True):
    ...
    opacity: float  # Opacity of the toolbar's background panel, in [0, 1]
...
@dataclasses.dataclass
class TimelineMessage(Message, include_in_scene_serialization=False):
...
@dataclasses.dataclass
class TimelineRemoveMessage(Message, include_in_scene_serialization=False):
...
@dataclasses.dataclass
class GuiChatAttachment:
    """Display-side view of a file attached to a chat message."""
    ...
    """(Private) Small JPEG preview for images; None for other files."""


@dataclasses.dataclass
class GuiChatEntry:
    """A single message displayed in a chat."""
    ...
    text: str
    """Message text. Assistant and system messages are rendered as markdown."""
    attachments: Tuple[GuiChatAttachment, ...]
...
@dataclasses.dataclass
class GuiChatConversationInfo:
    """Summary of a saved conversation, listed in the chat history drawer."""
...
@dataclasses.dataclass
class GuiChatProps(GuiBaseProps):
    greeting: str
    """Heading shown while the conversation is empty."""
    subtitle: str
    """Text shown below the greeting while the conversation is empty."""
    suggestions: Tuple[str, ...]
    """Suggested prompts shown as chips while the conversation is empty."""
    disclaimer: Optional[str]
    """Optional notice shown above the input while the conversation is empty."""
    placeholder: str
    ...
    """Text of the assistant reply currently being streamed, or None."""
    busy: bool
    """True while a reply is being generated; disables submission."""
    conversations: Tuple[GuiChatConversationInfo, ...]
...
@dataclasses.dataclass
class GuiChatMessage(_CreateGuiComponentMessage):
    container_uuid: str
    props: GuiChatProps
...
@dataclasses.dataclass
class GuiChatUpload:
    """A file attached to a chat submission, as sent by the client."""

    name: str
    mime_type: str
    _data: bytes
    """(Private) Raw file bytes."""
    _thumbnail: Optional[bytes]
    """(Private) Small JPEG preview for images, generated by the client."""


@dataclasses.dataclass
class GuiChatSubmitMessage(Message, include_in_scene_serialization=False):
    """Message sent from client->server when the user submits a chat message."""

    uuid: str
    text: str
    attachments: Tuple[GuiChatUpload, ...]


@dataclasses.dataclass
class GuiChatActionMessage(Message, include_in_scene_serialization=False):
...
@dataclasses.dataclass
class FolderSelectMessage(Message, include_in_scene_serialization=False):
